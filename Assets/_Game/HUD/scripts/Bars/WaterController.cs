...
public class WaterController : MonoBehaviour
{
    ...
    public GameObject waterBar;     // obj da barra de comida 

    #endregion

    #region references

    private Image imageWaterBar;     // referencia imagem a ser adquirida para usar o fill amount
    ...
    [SerializeField]private float indiceIncreasedTime;  // indice de acrescimo no tempo, toda vez que ela for chamada o tempo aumentará
    ...
    {
        yield return new WaitForSeconds(minutos * (minutoQuantity + indiceIncreasedTime));
        imageWaterBar.fillAmount -= reductionValue;

        if(indiceIncreasedTime >= indiceIncreaseTimeMax)   // limite para nao permitir que o tempo aumente demais
        {
            indiceIncreasedTime = 0.5f;
        }
